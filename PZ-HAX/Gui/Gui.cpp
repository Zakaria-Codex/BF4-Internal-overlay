...
void CGui::GUI_Draw_Elements()
{
	...
	{
		Render::DrawingSpectatorList();
	}
	if (g_Options.MISC.RADAR.radarwindow)
	{
		Render::DrawRadars();
	}
	if (g_Options.SCREEN.screen)
	{
		if(g_Options.SCREEN.enemyclose)
		Render::DrawClosedPlayer();
		if (g_Options.SCREEN.totalenemy)
		Render::DrawTotalPlayer();
	}
	Render::Crosshair();
	Render::memory();
